...
const csvFilename = 'pupil-attainment.csv';
...
function interrogateSPARQL(zones) {
    ...
    _.forEach(_.chunk(zones, chunk_size), (zone_chunk) => {
        queries.push(`
            ...
            SELECT ?year ?zone ?gender ?awards ?level ?stage ?measure ?value
            WHERE {
                ?s qb:dataSet data:pupil-attainment ;
                   qb:measureType ?m ;
                   ?m ?value ;
                   dim:gender ?g ;
                   dim:numberOfAwards ?a ;
                   dim:scqfLevel ?l ;
                   dim:secondarySchoolStage ?c ;
                   ldim:refArea ?z ;
                   ldim:refPeriod ?y .
                ?g rdfs:label ?gender .
                ?z rdfs:label ?zone .
